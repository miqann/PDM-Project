                ...
                if(result.length != 0){// if have result
                   ...
                   password == result[0].password ? state = 5 : state = 1;
                    loginLog.insertLog(result[0], password, state)
                            .then(state == 5 ? res.status(200).json({message: 'true', accountId: result[0].AccountId, currentBalance: result[0].CurrentBalance}):res.status(200).json({message: 'Invalid credentials'}));
                }
                else {
